...
def matxMultiply(A, B):
    ...
    row_B,col_B = shape(B)
    if(col_A != row_B):
        raise ValueError("can not matxMultiply")
    C = []
    for i in range(row_A):
        ...
        for j in range(col_B):
            ...
            for k in range(col_A):
                 sum_of_mutiply += A[i][k] * B[k][j]
            C[i].append(sum_of_mutiply)
    return C
...
#         A_transpose.append(b_transpose[i])
#     return transpose(A_transpose)
...
def addScaledRow(M, r1, r2, scale):
    M[r1]=[mr1 + mr2 * scale for mr1,mr2 in zip(M[r1],M[r2])]
...
#     0 & 0 & 0 & 0 \\
#     0 & \frac{15}{4} & -\frac{33}{8} & 6 \end{bmatrix}$
# 
# $ --> \begin{bmatrix}
#     1 & -\frac{3}{4} & -\frac{3}{8} & -\frac{1}{8} \\
#     0 & \frac{15}{4} & -\frac{33}{8} & 6 \\
#     0 & 0 & 0 & 0 \end{bmatrix}$
#     
# $ --> \begin{bmatrix}
#     1 & 0 & -\frac{9}{20} & -\frac{35}{8} \\
#     0 & 1 & -\frac{11}{10} & 6 \\
#     0 & 0 & 0 & 0 \end{bmatrix}$
#     
# $...$
...
def swap_max_abs_row(A,index):
    r,c = shape(A)
    max_index = index
    max_abs = abs(A[index][index])
    for k in range(index+1,r):
        if(abs(A[k][index])>max_abs):
            max_index,max_abs = k,abs(A[k][index])
           
    if(index != max_index):
        swapRows(A,index,max_index)
...
def calculateMSE2D(X,Y,m,b):
    ms = [(yi-m * xi-b) ** 2 for xi,yi in zip(X,Y)]
...
def linearRegression2D(X,Y):
    MC = [1 for i in range(len(X))]
    MX = transpose([X,MC])
    MY = transpose([Y])
    
    print_matrix(MX)
    
    XTX = matxMultiply(transpose(MX),MX)
    XTY = matxMultiply(transpose(MX),MY)
